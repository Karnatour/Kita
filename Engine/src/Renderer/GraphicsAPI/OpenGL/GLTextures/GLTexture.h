...
namespace Kita {
    ...
    public:
        ~GLTexture() override;
        std::expected<void, TextureError> createTexture(const std::optional<std::filesystem::path>& texturePath, TextureType textureType, std::optional<std::pair<int, int>> resolution) override;
